//! 한국투자증권 해외 주식 ExchangeProvider 구현.

use crate::connector::kis::client_us::KisUsClient;
...
/// 한국투자증권 해외 주식 ExchangeProvider 구현.
///
/// KisUsClient를 래핑하여 거래소 중립적인 ExchangeProvider 인터페이스를 제공합니다.
pub struct KisUsProvider {
    client: Arc<KisUsClient>,
}

impl KisUsProvider {
    /// 새 Provider 생성 (Arc로 래핑).
    pub fn new(client: Arc<KisUsClient>) -> Self {
        Self { client }
    }

    /// KisUsClient로부터 직접 생성 (Arc로 자동 래핑).
    pub fn from_client(client: KisUsClient) -> Self {
        Self::new(Arc::new(client))
    }
}

#[async_trait]
impl ExchangeProvider for KisUsProvider {
    async fn fetch_account(&self) -> Result<StrategyAccountInfo, ProviderError> {
        ...
        let balance = self
            ...
            .get_balance("USD")
            .await
            .map_err(|e| ProviderError::Api(format!("KIS US 잔고 조회 실패: {}", e)))?;

        // 계좌 요약 정보가 없으면 기본값 사용
        let summary = balance
            .summary
            .ok_or_else(|| ProviderError::Parse("KIS US 계좌 요약 정보 없음".to_string()))?;

        // 보유 종목 평가액 합계
        let total_position_value: Decimal = balance.holdings.iter().map(|h| h.eval_amount).sum();

        // 총 자산 = 평가액 (현물은 현금 잔고가 별도로 제공되지 않음, 평가액에 포함)
        let total_balance = summary.total_eval_amount.unwrap_or(total_position_value);

        // 미실현 손익 = 보유 종목 평가손익 합계
        let unrealized_pnl = summary
            .total_profit_loss
            .unwrap_or_else(|| balance.holdings.iter().map(|h| h.profit_loss).sum());

        Ok(StrategyAccountInfo {
            total_balance,
            available_balance: total_balance - total_position_value, // 근사값 (실제 현금 잔고)
    ...
    async fn fetch_pending_orders(&self) -> Result<Vec<PendingOrder>, ProviderError> {
        ...
        let pending_orders = self
            ...
            .get_pending_orders()
            .await
            .map_err(|e| ProviderError::Api(format!("KIS US 미체결 주문 조회 실패: {}", e)))?;
...
mod tests {
    #[test]
    fn test_kis_us_provider_creation() {
        // KisUsClient는 실제 API 키가 필요하므로 구조만 테스트
        // 통합 테스트는 mock을 사용
    }
}
